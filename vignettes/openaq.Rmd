...
An API key is required for using the OpenAQ API. Register for an account at [https://explore.openaq.org/register](https://explore.openaq.org/register) to get an API key.
...
```{r eval=FALSE}
set_api_key("my-super-secret-openaq-key-1234")
list_locations(api_key="this-is-my-alternate-api-key") # This key will override the previous one, for this call only.
```
...
by default in the openaq package as object attributes on the output date
frame:
...
the requestion would have been made. 
