# Authors and Citation

## Authors

- **Russ Biggs**. Author, maintainer.
  [](https://orcid.org/0009-0008-0777-8479)

- **Christian Parker**. Author.

- **Francesca Mills**. Contributor.

- **OpenAQ**. Copyright holder, funder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/openaq/openaq-r/blob/v1.1.0/DESCRIPTION)

Biggs R, Parker C (2026). *openaq: Access Air Quality Data from the
'OpenAQ' API*. R package version 1.1.0,
<https://github.com/openaq/openaq-r>.

    @Manual{,
      title = {openaq: Access Air Quality Data from the 'OpenAQ' API},
      author = {Russ Biggs and Christian Parker},
      year = {2026},
      note = {R package version 1.1.0},
      url = {https://github.com/openaq/openaq-r},
    }
